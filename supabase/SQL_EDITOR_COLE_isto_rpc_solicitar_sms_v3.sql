-- COLE TUDO no SQL Editor do Supabase (uma vez).
-- Pré-requisito: a função rpc_refresh_fidelity_level deve existir (migração 019). Se o Editor acusar erro, aplica antes supabase/migrations/019_rpc_refresh_fidelity_level.sql
-- Isto = migração 021 (cópia fiel). Confirma que rpc_solicitar_sms_v3 existe e fica alinhada ao repositório.
