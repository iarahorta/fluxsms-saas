-- 024: Chip marcado 'offline' pelo heartbeat mas com last_ping ou polo em ≤60 min entra na venda e no estoque.
-- Prioridade: cliente comprar quando há modem vivo; não exige polo.status = ONLINE.
...
CREATE OR REPLACE FUNCTION public.rpc_solicitar_sms_v3(
    ...
    p_registered_by_api_key_id UUID DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHERE COALESCE(c.chip_service_off ->> v_service_key, 'false') <> 'true'
      ...
      AND (
          p_registered_by_api_key_id IS NULL
          OR c.registered_by_api_key_id IS NULL
          OR c.registered_by_api_key_id = p_registered_by_api_key_id
      )
      AND (
          (v_service_key = 'whatsapp'
              AND (c.status = 'idle' OR (
                  lower(c.status::text) = 'offline'
                  AND (c.last_ping > NOW() - INTERVAL '60 minutes' OR p.ultima_comunicacao > NOW() - INTERVAL '60 minutes')
              ))
              AND (c.disponivel_em IS NULL OR c.disponivel_em <= NOW()))
          OR (v_service_key <> 'whatsapp'
              AND (
                  c.status IN ('idle', 'quarentena')
                  OR (
                      lower(c.status::text) = 'offline'
                      AND (c.last_ping > NOW() - INTERVAL '60 minutes' OR p.ultima_comunicacao > NOW() - INTERVAL '60 minutes')
                  )
