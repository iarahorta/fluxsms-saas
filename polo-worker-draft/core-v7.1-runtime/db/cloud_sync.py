...
def _safe_load_dotenv():
    """Evita crash do load_dotenv/find_dotenv em runtime empacotado."""
...
# Carrega variáveis de ambiente (sem derrubar o core)
_safe_load_dotenv()
...
_api_mode = False
_api_base = (os.getenv("BACKEND_URL") or "https://fluxsms.com.br").rstrip("/")
_api_key = (os.getenv("PARTNER_API_KEY") or "").strip()
_api_hwid = (os.getenv("FLUXSMS_HWID") or "").strip()

def _partner_api_headers():
    h = {
        "Authorization": f"Bearer {_api_key}",
        "Content-Type": "application/json"
    }
    if _api_hwid:
        h["X-Flux-Hwid"] = _api_hwid
    return h

def _partner_api_ready():
    return bool(_api_key and _api_base)

def _partner_api_call(method, path, json_data=None, timeout=20):
    url = f"{_api_base}{path}"
    return requests.request(
        method=method,
        url=url,
        headers=_partner_api_headers(),
        json=json_data,
        timeout=timeout
    )
...
def init_polo():
    ...
    global _current_polo_id, _api_mode
    if (not SUPABASE_URL or not SUPABASE_KEY) and _partner_api_ready():
        try:
            resp = _partner_api_call("GET", "/partner-api/worker/bootstrap")
            if resp.status_code >= 200 and resp.status_code < 300:
                body = resp.json() if resp.text else {}
                polo = body.get("polo") or {}
                _current_polo_id = polo.get("id")
                _api_mode = True
                polo_nome = polo.get("nome") or "Polo Ativo"
                print(f"  ☁️  [API] Bootstrap parceiro OK ({polo_nome})")
                return True, polo_nome
            print(f"⚠️ Bootstrap parceiro falhou: HTTP {resp.status_code} {resp.text[:240]}")
            return False, None
        except Exception as e:
            print(f"⚠️ Erro na API do parceiro ao inicializar polo: {e}")
            return False, None
    if not SUPABASE_URL or not SUPABASE_KEY:
        print("⚠️ Erro: SUPABASE_URL ou SUPABASE_SERVICE_KEY ausentes no .env, e PARTNER_API_KEY não foi informada.")
        return False, None
...
def sync_chip_to_cloud(porta, numero, operadora):
    ...
    if _api_mode:
        ...
        try:
            resp = _partner_api_call("POST", "/partner-api/worker/sync", json_data=payload)
            if resp.status_code in (200, 201):
                print(f"  [OK] Chip {numero} enviado para a nuvem com sucesso! (Atualizado na {porta})")
            else:
                print(f"  ⚠️ Erro sync parceiro: HTTP {resp.status_code} {resp.text[:220]}")
        except Exception as e:
            print(f"  ⚠️  [API] Erro de rede ao sincronizar chip: {e}")
        return
...
def set_all_offline():
    ...
    if _api_mode:
        if not _partner_api_ready():
            return
        try:
            _partner_api_call("POST", "/partner-api/worker/shutdown", json_data={})
            print("  ☁️  [API] Polo/chips setados como OFFLINE")
        except Exception as e:
            print(f"Erro offline API: {e}")
        return
    if not _current_polo_id or not SUPABASE_URL: return
...
def heartbeat_polo():
    ...
    if _api_mode:
        if not _partner_api_ready():
            return
        try:
            _partner_api_call("POST", "/partner-api/worker/heartbeat", json_data={})
        except:
            pass
        return
    if not _current_polo_id or not SUPABASE_URL: return
